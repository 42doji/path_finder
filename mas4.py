...
# 4. 데이터 병합 및 재구성 (핵심 수정 부분)
# 4-1. area_struct에서 Area 1 데이터만 필터링합니다.
area_1_structs = area_struct[area_struct['Area_id'] == 1].copy()

# 4-2. (x, y) 좌표를 기준으로 area_map과 area_1_structs를 병합합니다.
# 'how='outer''는 양쪽의 모든 좌표를 포함하기 위함입니다.
area_1_df = pd.merge(area_map, area_1_structs, on=['x', 'y'], how='outer')

# 4-3. 최종 구조물 이름을 결정하는 규칙을 적용합니다.
# 'ConstructionSite'가 1이면 '건설 현장', 아니면 'Struct_nm' 값을 사용합니다.
def get_final_struct_name(row):
    if row['ConstructionSite'] == 1:
        return '건설 현장'
    return row['Struct_nm']

area_1_df['Final_Struct'] = area_1_df.apply(get_final_struct_name, axis=1)
...
for _, row in area_1_df.iterrows():
    # 최종적으로 결정된 구조물 이름('Final_Struct')을 사용합니다.
    x, y, struct_name = row['x'], row['y'], row['Final_Struct']
    
    if pd.isna(struct_name): # 병합 후 이름이 없는 데이터는 건너뜁니다.
        continue
...
max_coord = int(max(area_1_df['x'].max(), area_1_df['y'].max()) + 2)
